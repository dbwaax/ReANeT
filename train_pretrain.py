...
from model.GANMRF_dup import CEANet_res50
...
if torch.cuda.device_count() > 1:
    ...
    # generate = BalancedDataParallel(4, generate, device_ids=[0,1])
...
for epoch in range(1,args.Epoch+1):
    ...
    if(epoch % 1 == 0 and epoch!=0):
        ...
        if(ACC<Accuracy):
            ACC = Accuracy
            if( ACC > 0.95 ):
                torch.save(generate.state_dict(), args.saved_path + '/' + 'CEAFomer_res50_{:.5f}_{}.pkl'.format(ACC,epoch))  

torch.save(generate.state_dict(), args.saved_path + '/' + 'CEAFomer_res50_final.pkl')  